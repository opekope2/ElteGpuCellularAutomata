...
# Flags
DEFINES=(-DCL_HPP_ENABLE_EXCEPTIONS -DCL_HPP_MINIMUM_OPENCL_VERSION=120 -DCL_HPP_TARGET_OPENCL_VERSION=300 -DCL_TARGET_OPENCL_VERSION=300 -DOS="$OS")
CXXFLAGS=(-std=c++20)
LDFLAGS=(-lOpenCL)
BUILD_FLAGS=(-O2)
DEBUG_FLAGS=(-g -O0)
...
_build() (
    __run "$CXX" -Wall -o "$OUT"/cellular_automata "$@" "$SRC"/*.cpp "$GEN"/*.cpp ${CXXFLAGS[@]} ${LDFLAGS[@]} ${DEFINES[@]}
)
